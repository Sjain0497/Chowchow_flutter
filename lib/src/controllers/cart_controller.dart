...
class CartController extends ControllerMVC {
  ...
  List<Cart> cartsNew = <Cart>[];
  double taxAmount = 0.0;
  double taxAmountNew = 0.0;
  double deliveryFee = 0.0;
  double deliveryFeeNew = 0.0;
  int cartCount = 0;
  double subTotal = 0.0;
  ...
  void listenForCartsAgain({String message}) async {
    cartsNew.clear();
    final Stream<Cart> stream = await getCartAgain();
    stream.listen((Cart _cart) {
      if (!cartsNew.contains(_cart)) {
        setState(() {
          coupon = _cart.food.applyCoupon(coupon);
          cartsNew.add(_cart);
        });
      ...
      if (cartsNew.isNotEmpty) {
        calculateSubtotalAgain();
      }
      if (message != null) {
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(message),
  ...
  void listenForPayments(Cart cart) async {
    getPayment(cart);
  }
  ...
  void calculateSubtotalAgain() async {
    double cartPrice = 0;
    subTotalNew = 0;
    cartsNew.forEach((cart) {
      ...
      cartPrice *= cart.quantity;
      subTotalNew += cartPrice;
    });
    if (Helper.canDelivery(cartsNew[0].food.restaurant, carts: cartsNew)) {
      deliveryFeeNew = cartsNew[0].food.restaurant.deliveryFee;
    }
    taxAmountNew = (subTotalNew + deliveryFeeNew) *
        cartsNew[0].food.restaurant.defaultTax /
        100;
    totalNew = subTotalNew + taxAmountNew + deliveryFeeNew;
    setState(() {});
  }
...
//          });
  ...
  void goCheckout(BuildContext context, [List<Cart> carts]) {
    if (!currentUser.value.profileCompleted()) {
      ...
      if (carts[0].food.restaurant.closed) {
        ...
        Navigator.of(context).pushNamed('/PaymentMethod', arguments: carts);
      }
    }
  }
