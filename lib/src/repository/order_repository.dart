...
import 'package:chowchow/src/models/cart.dart';
...
Future<Stream<Order>> getOrders() async {
  ...
  final String url =
      ...
      '${GlobalConfiguration().getValue('api_base_url')}allorder?user_id=${_user.id}';
  print("OrderData...$url");
...
Future<Stream<Order>> getOrder(orderId) async {
  ...
  final String url =
      ...
      '${GlobalConfiguration().getValue('api_base_url')}getorder/$orderId';
...
Future<Stream<OrderStatus>> getOrderStatus() async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String _confirm = 'confirm=true';
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}order_status?$_apiToken&$_confirm';
  print("OrderStatus...$url");
  final client = new http.Client();
...
Future<Stream<Order>> addOrder(Order order, Payment payment, Cart cart) async {
  User _user = userRepo.currentUser.value;
  if (_user.apiToken == null) {
    return new Stream.value(null);
  }
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String _itemId = 'item_id=${cart.id}';
  final String _userid = 'user_id=${_user.id}';
  final String _orderStatusid = 'order_status_id=1';
  print("UserId..$_userid");
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}orders?$_userid&$_orderStatusid';
  print("AddOrderUrl...$url");
  final client = new http.Client();
  Map params = order.toMap();
  //params.addAll(_creditCard.toMap());
  final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
  print("OrderStatusId....${(streamedRest)}");
  return streamedRest.stream
      .transform(utf8.decoder)
      .transform(json.decoder)
      .map((data) => Helper.getData(data))
